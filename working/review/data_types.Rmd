...
# we can use sample to simulate a random recording of catch by fisherman, let's say we pick 20 fish from the net
...
ggplot(data.frame(catch1=catch1), 
       aes(catch1, fill=catch1)) + 
...
Let's generate a function that makes use of categorical data. Species diversity is a good example.
...
catch1 = sample(possible.fish, size=10, prob = c(0.2, 0.2, 0.1, 0.1, 0.4),
                replace=T)

# let's create a test case that should have low diversity, by repeating the same thing
...
Sometimes you don't want factors and R thinks something should be one
...
# sum(a) doesn't work; cannot sum factors
...
Let's start with diversity - maybe want to know a bit more about the dataset
...
# repeat with a list
source("C:/Users/deann/Box/Winter 2023/esm_262/R/compute_diversity.R")
