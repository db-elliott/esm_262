...
What could be the total power consumed if there are 100 cars on this highway each hour, they are travelling at a range of speeds-mean is 100km/hr--and speeds tend to
...
# let's get a sample of 200 speeds- we could also do this by actually measuring speeds
...
# for plotting sometimes it's useful to turn columns into rows - we can use an R function called pivot_longer (part of the tidyverse package) to do this
...
# quick visualization
# let's save it so that we can compare
...
# recall our function needs speed in m/s not km/hr so we will also do a conversion
# let's get a sample of a 200 speeds- we could also do this by actually measuring speeds
...
# 'apply' family of functions does this too - FYI
...
Other programming languages (including R) use what are called loops - where repetition is more explicit
...
# for plotting sometimes it's useful to turn columns in to rows
